...
const { sendEmail } = require('./scripts/smtp-mailer');
...
function getDevServerConfig() {
    return {
        ...
        proxy: [
            {
                context: path => !path.startsWith('/api/send-email'),
        ...
        setupMiddlewares: (middlewares, devServer) => {
            if (devServer?.app) {
                devServer.app.post('/api/send-email', async (req, res) => {
                    try {
                        const payload = await readJsonRequestBody(req);
                        const { body, subject, to } = payload;
            ...
            return middlewares.filter(m => m.name !== 'cross-origin-header-check');
        },
        static: {
