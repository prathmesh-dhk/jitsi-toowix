...
export function ShareRecordingModal({
  ...
  const addFromContacts = (emails: string[]) => {
    const nextList = Array.from(new Set([...sharedList, ...emails]));
    setSharedList(nextList);
    persistChanges({ sharedWith: nextList });
  };
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: 'var(--color-text-primary)', letterSpacing: '-0.01em' }}>
                Share "{recording.name}"
              </h3>
              <p style={{ margin: '3px 0 0', fontSize: '12px', color: 'var(--color-text-secondary)' }}>
                Choose who can view this recording
              </p>
            </div>
          </div>
        ...
        <div style={{ padding: '20px 22px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: 650, color: 'var(--color-text-primary)', marginBottom: '7px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button
                ...
                style={{
                  padding: '0 14px',
                  borderRadius: 'var(--radius-md)',
                  background: emailInput.trim() ? 'var(--color-primary)' : 'var(--color-bg-subtle)',
                  color: emailInput.trim() ? '#FFFFFF' : 'var(--color-text-muted)',
                  border: 'none',
                  fontSize: '13px',
                  fontWeight: 600,
          ...
          <div style={{ marginTop: '-8px' }}>
            <button
              ...
              style={{ background: 'none', border: 'none', color: 'var(--color-primary)', fontSize: '12.5px', fontWeight: 650, cursor: 'pointer', padding: 0 }}
            >
              + Choose from contact book
            </button>
            <ContactPicker isOpen={showContactPicker} onClose={() => setShowContactPicker(false)} alreadyAdded={sharedList} onAdd={addFromContacts} />
          </div>

          {/* People with access list */}
          <div>
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', marginBottom: '8px', letterSpacing: '0.05em' }}>
              People with access
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '120px', overflowY: 'auto' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: 'var(--color-primary)', color: '#FFFFFF', fontSize: '11px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {sharedList.map((email) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: 'var(--color-primary-light)', color: 'var(--color-primary)', fontSize: '11px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {email[0].toUpperCase()}
                    </div>
                    <span style={{ fontSize: '12.5px', color: 'var(--color-text-primary)' }}>{email}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ fontSize: '11px', color: 'var(--color-text-muted)', fontWeight: 600 }}>Viewer</span>
                    <button
                      type="button"
                      onClick={() => handleRemoveEmail(email)}
                      title="Remove access"
                      style={{ border: 'none', background: 'transparent', color: 'var(--color-error)', cursor: 'pointer', padding: '2px' }}
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div
                  style={{
                    ...
                    background: allowShare ? 'var(--color-primary-light)' : 'var(--color-surface-hover)',
                    color: allowShare ? 'var(--color-primary)' : 'var(--color-text-muted)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 650, color: 'var(--color-text-primary)' }}>
                    {allowShare ? 'Anyone with the link' : 'Restricted'}
                  </div>
                  <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)', lineHeight: 1.35, marginTop: '2px' }}>
                    {allowShare
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${!allowShare ? 'var(--color-primary-border)' : 'var(--color-border)'}`,
                    background: !allowShare ? 'var(--color-primary-light)' : 'var(--color-surface)',
                    color: !allowShare ? 'var(--color-primary)' : 'var(--color-text-secondary)',
                    fontSize: '11.5px',
                    fontWeight: 600,
                    cursor: 'pointer',
                  }}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 'var(--radius-md)',
              border: 'none',
              background: copiedMessage ? 'var(--color-success)' : 'var(--color-primary)',
              color: '#FFFFFF',
              fontSize: '13.5px',
              fontWeight: 650,
              display: 'flex',
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 'var(--radius-sm)',
                border: 'none',
                background: copiedLink ? 'var(--color-success)' : 'var(--color-primary)',
                color: '#FFFFFF',
                fontSize: '12px',
                fontWeight: 600,
                cursor: 'pointer',
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
              ...
              >
                ...
                <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)', fontWeight: 600 }}>WhatsApp</span>
              </button>
              ...
              >
                ...
                <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)', fontWeight: 600 }}>Gmail</span>
              </button>
              ...
              >
                ...
                <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)', fontWeight: 600 }}>Telegram</span>
              </button>
              ...
              >
                ...
                <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)', fontWeight: 600 }}>More apps</span>
              </button>
            </div>
          </div>
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '12px', color: saveSuccess ? 'var(--color-success)' : 'var(--color-text-muted)', fontWeight: saveSuccess ? 600 : 400 }}>
